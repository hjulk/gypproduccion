        ...
        <div id="carruselPrincipal" class="carousel slide carousel-fade" data-ride="carousel">
            ...
            <ol class="carousel-indicators" tabindex="0">
                ...
                @for($ContadorCM = 0; $ContadorCM < $ContadorCarruselMovil;$ContadorCM++)
                    <li data-target="#carruselPrincipal" data-slide-to="{{ $ContadorCM}}" class="@if($ContadorCM === 0) active @endif">
                    </li>
                    @endfor
                    @endif
            </ol>
            ...
            <ol class="carousel-indicators" tabindex="0">
                ...
                @for($Contador = 0; $Contador < $ContadorCarrusel;$Contador++)
                    <li data-target="#carruselPrincipal" data-slide-to="{{ $Contador}}" class="@if($Contador === 0) active @endif">
                    </li>
                    @endfor
                    @endif
            </ol>
            ...
            <div class="col-md-8">
                <p id="tarifasText" tabindex="0">
                    ...
                    <a href="servicios/tarifas" id="tarifasEnlace">
                        @if($TarifasAnio)
                        @foreach($TarifasAnio as $imagesTA)
                        @if(strpos($imagesTA->UBICACION, '.jpg') !== false)
                        <picture tabindex="0">
                            <source srcset="{{  asset(str_replace('../', '', $imagesTA->UBICACION_WEBP)) }}" type="image/webp" />
                            <source srcset="{{ asset(str_replace('../', '/', $imagesTA->UBICACION)) }}" type="image/jpg" />
                            <img src="{{ asset(str_replace('../', '/', $imagesTA->UBICACION_WEBP)) }}" alt="Nuestras Tarifas" />
                        </picture>
                        @else
                        <picture tabindex="0">
                            <source srcset="{{ asset(str_replace('../', '/', $imagesTA->UBICACION_WEBP)) }}" type="image/webp" />
                            <source srcset="{{ asset(str_replace('../', '/', $imagesTA->UBICACION)) }}" type="image/png" />
                            <img src="{{ asset(str_replace('../', '/', $imagesTA->UBICACION_WEBP)) }}" alt="Nuestras Tarifas" />
                        </picture>
                        @endif
                        @endforeach
